// extensions/checkout-ui/src/Checkout.jsx  (wholesale)
//
// Processing fee — 3% of cart GRAND TOTAL (items + shipping + tax) —
// added at checkout as ONE real cart line item.
//
// Mirrors ns-retail/extensions/processing-fee/src/Checkout.jsx 1:1 —
// same on-demand exact-price variant model. When you change one of
// these two files, update the other to match.
//
// ── On-demand exact-price variant model ──────────────────────────────
//
// Extension computes the desired fee, POSTs it to the wholesale app
// backend (`/api/cdo/fee-variant`), and receives the GID of a
// Processing Fee variant priced at EXACTLY that amount. Backend
// creates the variant on cache miss and LRU-evicts the oldest one
// when the product hits Shopify's 2000-variant-per-product cap.
//
// Trade-off vs the previous tier picker:
...
//        (Shopify snapshots line-item data at order time).
//
// ── Two compensation layers ──────────────────────────────────────────
//
// (1) Self-compensation (fee-on-fee):
//     realCartTotal = totalAmount − Σ feeLine.cost.totalAmount
...
//
// ── Env / setup (per-environment, one-time) ──────────────────────────
//   1. Create the "Processing Fee" product in the wholesale store.
//   2. Set env var `PROCESSING_FEE_PRODUCT_ID` on the wholesale app
...
// session. Used to identify "our" cart lines regardless of which price
...
// ── Backend fetch: resolve/create an exact-price fee variant ─────────
//
// POST { price } to /api/cdo/fee-variant on the wholesale app backend.
// Returns { price, gid, source } where source ∈ {cache, cache-inlock,
// created}. Throws on network / server failure.
/**
 * @param {number} price
 * @returns {Promise<{ price: number, gid: string, source: string }>}
 */
async function fetchFeeVariant(price) {
  const base = FullPageApi.getAppBaseUrl()
  if (!base) {
    throw new Error('App backend URL is not configured')
  }
  const res = await fetch(`${base}/api/cdo/fee-variant`, {
    method: 'POST',
    headers: {
...
function ProcessingFee() {
  useSignalEffect(() => {
    ...
    if (isMutating) {
      ...
      console.log(
        '[processing-fee:wholesale] ⏸️  skip — mutation cycle already in flight',
      )
      return
    }
    ...
    if (!canAdd) {
      // eslint-disable-next-line no-console
      console.log(
        '[processing-fee:wholesale] ⏸️  skip — canAddCartLine=false',
      )
      return
    }
    ...
    if (activeTargetPriceStr.value === targetPriceStr && !existingFeeLine) {
      // eslint-disable-next-line no-console
      console.log(
        `[processing-fee:wholesale] ⏸️  provisioning $${targetPriceStr} — awaiting result`,
      )
      return
    }

    // GID-based no-swap check — the fee line's variant IS the one we
    // want if its merchandise.id matches the cached gid for this
    // target price. Falls back to reconstructed-price epsilon.
    if (existingFeeLine) {
    ...
    // eslint-disable-next-line no-console
    console.log(
      `[processing-fee:wholesale] 💵 totalAmount=$${totalAmount.toFixed(2)} · realCart=$${realCartTotal.toFixed(2)} · discountOnFee=${(capped * 100).toFixed(2)}% · desiredNet=$${desiredNet.toFixed(2)} · target=$${targetPriceStr}`,
    )
    ...
    fetchFeeVariant(Number(targetPriceStr))
      .then((result) => {
        const gid = result?.gid
        const priceStr = formatPrice(
          Number(result?.price ?? targetPriceStr),
        )
        if (!gid) {
          // eslint-disable-next-line no-console
          console.error(
            '[processing-fee:wholesale] ❌ backend returned no gid for target=$' +
              targetPriceStr,
          )
      ...
      .catch((err) => {
        // eslint-disable-next-line no-console
        console.error(
          `[processing-fee:wholesale] ❌ fetchFeeVariant($${targetPriceStr}) failed:`,
          err?.message || err,
        )
        activeTargetPriceStr.value = null
      })
...
function applyTarget(targetPriceStr, targetGid, feeLines) {
  ...
  if (mutations.length === 0) {
    // No mutations needed but do NOT write activeTargetPriceStr —
    // that would retrigger the effect. Leave state as-is; the
  ...
  ;(async () => {
    try {
      for (const m of mutations) {
        // eslint-disable-next-line no-console
        console.log(
          `[processing-fee:wholesale] → ${m.type}${'merchandiseId' in m ? ' ' + m.merchandiseId : ' id=' + m.id} qty=${m.quantity}`,
        )
        await shopify.applyCartLinesChange(m)
      }
      // eslint-disable-next-line no-console
      console.log(
        `[processing-fee:wholesale] ✅ ${label} — ${mutations.length} step${mutations.length === 1 ? '' : 's'}`,
      )
    } catch (/** @type {any} */ err) {
      // eslint-disable-next-line no-console
      console.error(
        `[processing-fee:wholesale] ❌ ${label} failed:`,
        err?.message || err,
      )
    } finally {
      isMutating = false
