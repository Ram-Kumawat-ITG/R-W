...
import { ensureCustomerForOrder } from '../customers/ensureCustomer.server'
import { createInvoiceForOrder, attemptInvoiceCharge } from '../invoices/invoiceService.server'
import { validateShopifyOrder } from './validateOrder.server'
import { config } from '../config.server'
import { createLogger } from '../logger.server'

const log = createLogger('orders.process')

// Status that mean "no work to redo" — re-deliveries return early.
const TERMINAL_STATUSES = new Set(['completed', 'invoiced', 'scheduled', 'rejected'])
// Status that mean "another worker owns this right now".
...
export async function processShopifyOrder({ shop, order, webhookId }) {
  ...
  try {
    ...
    if (config.payments.chargeImmediately) {
      console.log(`[orders] step 3/4 — immediate NMI charge (PAYMENT_CHARGE_IMMEDIATELY=true)`)
      const chargeResult = await attemptInvoiceCharge({ invoice, customerMap })
      console.log(
