...
async function readTokenDoc() {
  return QboToken.findOne({ realmId: config.qbo.realmId }).lean()
}
...
async function bootstrapTokenDocFromEnv() {
  ...
  if (!config.qbo.bootstrapRefreshToken) {
  ...
  const seedDoc = await QboToken.findOneAndUpdate(
    { realmId: config.qbo.realmId },
    {
      $setOnInsert: {
        realmId: config.qbo.realmId,
        refreshToken: config.qbo.bootstrapRefreshToken,
        accessToken: 'pending',
...
async function refreshAccessToken(currentRefreshToken) {
  ...
  inFlightRefresh = (async () => {
    ...
    const res = await fetch(config.qbo.oauthTokenUrl, {
      method: 'POST',
      headers: {
...
function buildUrl(path, query) {
  const base = `${config.qbo.apiBaseUrl}/v3/company/${config.qbo.realmId}`
...
// Convenience helpers — used by customer/invoice modules.
export const qbo = {
