...
//
// Bakes the current app URL into the Customer Account UI extensions' shared
// HTTP client (extensions/services/FullPageApi.jsx → `const SERVER_URL = "..."`).
//
// WHY THIS EXISTS
// ---------------
// The Practitioner Portal is a Customer Account UI extension that runs in a
// Web Worker sandbox. That sandbox has NO `process.env`, so the absolute
// backend URL it fetch()es (`${SERVER_URL}/api/portal/<name>`) cannot be read
...
//
// It runs automatically from the `predev` and `predeploy` npm hooks. Ported
// from the equivalent script in the sibling `wholesale` workspace.
//
...
//   3. SHOPIFY_APP_URL in ns-retail/.env   (the Shopify CLI rewrites this each
