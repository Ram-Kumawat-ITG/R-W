...
//
//        discountRateOnFee = (qty × 0.01 − feeTotal) / (qty × 0.01)
...
function ProcessingFee() {
  ...
  useSignalEffect(() => {
    ...
    // eslint-disable-next-line no-console
    console.log(
      `[processing-fee:retail] 💵 totalAmount=$${totalAmount.toFixed(2)} · realCartTotal=$${realCartTotal.toFixed(2)} · discountOnFee=${(discountRateOnFee * 100).toFixed(2)}% · desiredNet=$${desiredFeeNet.toFixed(2)} · inflated=$${(desiredFeeNet * inflationFactor).toFixed(2)} · targetQty=${targetQty} · existingQty=${existingFeeLine?.quantity ?? 0}`,
    )
